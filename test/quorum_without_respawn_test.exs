defmodule Swarm.QuorumWithoutRespawnTests do
  ...
  setup_all do
    ...
    Application.put_env(:swarm, :inject_start_reason, true)
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:swarm, :inject_start_reason)
  ...
  describe "net split" do
    ...
    test "should have process on each node before it happens" do
      ...
      Enum.each(@names, fn name ->
        {:ok, _pid} = register_name(@node1, name, MyApp.WorkerSup, :register, [self()])
      end)
    ...
    test "for inject_start_reason: true reason for register will be injected as first argument" do
      # start worker for each name
      Enum.each(@names, fn name ->
        {:ok, _pid} = register_name(@node1, name, MyApp.WorkerSup, :register, [self()])
      end)

      # ensure that for each name we got register call with :normal mode
      1..5
      |> Enum.each(fn _ -> assert_receive({:register_worker, :normal}) end)
      ...
      # ensure we got :failover try of registration for all workers on smaller partition
      (1..Enum.count(names_on_smaller_partition))
      |> Enum.each(fn _ -> assert_receive({:register_worker, :failover}) end)
